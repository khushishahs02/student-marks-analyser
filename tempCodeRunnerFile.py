...
def heatmap(input_file):
    ...
    sns.heatmap(
        ...
        yticklabels=[f"Student {i+1}" for i in range(len(marks_matrix))]
    )
...
def report_cards(students, subject_lists) :
        ...
        for s in students :
            ...
            pdf.set_font("Arial", "B", 16)
            pdf.cell(0, 10, "ACADEMIC REPORT CARD", ln=True, align="C")
            pdf.ln(10)

            pdf.set_font("Arial", size=12)

            pdf.cell(0, 8, f"Student ID: {sid}", ln=True)
            pdf.cell(0, 8, f"Name: {name}", ln=True)
            pdf.cell(0, 8, f"Rank: {rank_dic[sid]}", ln=True)
            ...
            for subject, marks in student_marks:
              pdf.cell(0, 8, f"{subject}: {marks}", ln=True)

            pdf.ln(5)
            pdf.multi_cell(0, 8, f"Remark: {remark}")
